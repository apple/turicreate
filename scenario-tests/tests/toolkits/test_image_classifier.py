...
def _get_data(num_examples=100, label_type=int):
    ...
    if label_type == int:
        ...
        random_labels = [
            rs.choice(["a", "b", "c", "d", "e"]) for _ in range(num_examples)
        ]
...
class ImageClassifierTest(unittest.TestCase):
    ...
    def setUpClass(
        ...
        self.model = tc.image_classifier.create(
            self.sf, target=self.target, model=self.pre_trained_model, seed=42
        )
    ...
    def test_create_with_missing_feature(self):
        with self.assertRaises(_ToolkitError):
            tc.image_classifier.create(
                self.sf, feature="wrong_feature", target=self.target
            )

    def test_create_with_missing_label(self):
        with self.assertRaises(RuntimeError):
            tc.image_classifier.create(
                self.sf, feature=self.feature, target="wrong_annotations"
            )
    ...
    @unittest.skipIf(
        sys.platform == "darwin",
        "test_export_coreml_with_predict(...) covers this functionality and more",
    )
        ...
        for in_gpus in gpu_options:
            for out_gpus in gpu_options:
                ...
                model = tc.image_classifier.create(
                    self.sf, target=self.target, model=self.pre_trained_model
                )
...
class ImageClassifierSqueezeNetGPUTest(unittest.TestCase):
    @classmethod
    def setUpClass(self):
        super(ImageClassifierSqueezeNetGPUTest, self).setUpClass(
            model="squeezenet_v1.1"
        )
