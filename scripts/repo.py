...
def fix_headers_cmd(header_name, true_header):
    ...
    return (r"git grep --null -l '%s' %s | xargs -0 sed -i '' -e %s "
...
def fix_headers(filenames):
    ...
    for filename in filenames:
        ...
        header_files = [fn for fn in raw_header_list if fn.endswith("/" + filename)]
    ...
    shell_cmd = "\n".join(
             "{2} || echo 'ERROR fixing {0}; ignoring.' && echo 'Fixed {0} (True = {1}). ' \n".format(header, new_file, fix_headers_cmd(header, new_file))
