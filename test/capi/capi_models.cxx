...
BOOST_AUTO_TEST_CASE(test_call_function) {
  ...
  tc_variant* result =
      tc_model_call_function("_distances.manhattan", arguments, &error);
  TS_ASSERT(error == nullptr);
  ...
  // Providing a bogus name should generate an error.
  tc_variant_destroy(result);
  result = tc_model_call_function("b0gus 5unct10n nam3", arguments, &error);
  TS_ASSERT(error != nullptr);
