...
struct test_untranslated_coulmns  {
 public:
  ...
  void run_check(const std::string& run_string, const std::vector<size_t>& untranslated_columns) {
    ...
    std::map<std::string, v2::ml_column_mode> mode_overrides;

    for(size_t c_idx : untranslated_columns)
      mode_overrides[raw_data.column_name(c_idx)] = v2::ml_column_mode::UNTRANSLATED;

    // Several different versions of the ml_data, through various
    // stages of loading and unloading.
    std::array<v2::ml_data, 4> data_v;

    data_v[0].set_data(raw_data, "", {}, mode_overrides);
    data_v[0].fill();
            ...
            for(size_t type_idx : {0, 1, 2} ) {
              ...
              switch(type_idx) {
                case 0: {
                  ...
                  row_x_buffer = data.translate_row_to_original(x);
                  break;
                }
                case 1: {
              ...
              it.fill_untranslated_values(untranslated_row);
