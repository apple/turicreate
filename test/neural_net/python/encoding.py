...
mps_input = np.random.random_sample((1, 9, 9, 3))
...
class TestingEncoding(HybridBlock):
    def __init__(self):
        super(TestingEncoding, self).__init__(prefix='encoding_')
        with self.name_scope():
            self.conv1 = nn.Conv2D(32, 9, 1, 4, in_channels=3, use_bias=False)
            self.inst_norm1 = InstanceNorm(in_channels=32, num_styles=8, batch_size=1)
...
json_weights = {
    ...
    "encode_inst_gamma": transformer_weight_dict["encoding_instancenorm0_gamma"].flatten().tolist()
}
...
with open('data/encode/test_1/outputs.json', 'w') as fp:
    json.dump(json_outputs, fp)

with open('data/encode/test_1/inputs.json', 'w') as fp:
