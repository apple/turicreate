...
#include <toolkits/recsys/ml_data_2/side_features.hpp>
...
inline ml_data_internal::column_metadata_ptr
  ...
  return (side_features != nullptr
          ? side_features->get_full_column_metadata()[column_index]
 ...
 */
inline size_t ml_metadata::column_index(const std::string& _column_name) const {
  auto it = _column_name_to_index_map.find(_column_name);

  if(UNLIKELY(it == _column_name_to_index_map.end()))
 ...
 *  side columns.
 */
inline size_t ml_metadata::num_columns(bool include_side_columns_if_present,
                                       bool include_untranslated_columns) const {
 ...
 */
inline bool ml_metadata::has_side_features() const {
  return side_features != nullptr;
}

/** Returns the side feature storage class.
 */
std::shared_ptr<ml_data_side_features> ml_metadata::get_side_features() const {
  DASSERT_TRUE(has_side_features());
  return side_features;
}

/** Returns the current options.
 */
const std::map<std::string, flexible_type>& ml_metadata::get_current_options() const {
  return options;
}
