...
#include <toolkits/recsys/ml_data_2/ml_data_entry.hpp>
#include <toolkits/recsys/ml_data_2/ml_data_missing_values.hpp>
#include <toolkits/recsys/ml_data_2/ml_data_column_modes.hpp>
#include <toolkits/recsys/ml_data_2/data_storage/internal_metadata.hpp>
...
size_t fill_row_buffer_from_column_buffer(
    ...
    const std::vector<size_t>& index_remapping = std::vector<size_t>()) GL_HOT;
 ...
 *  feature_index, double value) tuples.
 *
 *  EntryWriteFunction out_function is a function taking arguments (size_t column_index,
 ...
 *  rm is the row_metadata structure associated with this row.
 */
template <typename EntryWriteFunction,
          typename ColumnAdvancementFunction,
          typename MLDataSideFeaturesType>   // Use a template here as
...
read_ml_data_row(
    ...
    EntryWriteFunction out_function,
    ColumnAdvancementFunction next_column,
    const std::shared_ptr<MLDataSideFeaturesType>& side_features) {
  ...
  // for y.
  {
    auto out_function_with_hook =
        [&](ml_column_mode mode, size_t column_index,
            size_t feature_index, double value,
            size_t index_size, size_t index_offset) GL_GCC_ONLY(GL_HOT_INLINE_FLATTEN) {

      out_function(mode, column_index, feature_index, value, index_size, index_offset);
    };

    _read_raw_ml_data_column_block(rm, row_block_ptr, out_function_with_hook, next_column);
  }
  ...
  if(use_side_features) {
    ...
    // to be done by supplying the function called at each entry,
    // since we'll just read that data again.
      ...
      auto out_function_to_add_side_data = [&](
        ...
        if(mode == ml_column_mode::CATEGORICAL) {
          ...
          if(data.row_block_ptr == nullptr) {
            ...
            // we still need to call next_column on these so the
            // function calling this can maintain state.
            for(size_t i = 0; i < data.rm.num_x_columns; ++i) {
              next_column(data.rm.metadata_vect[i]->mode,
          ...
          // Case 2: join on the data.
            ...
            // out_function on every element now.  However, the
            // column_offset needs to match the overall columns, not
            ...
            GL_GCC_ONLY(GL_HOT_INLINE_FLATTEN) {
              ...
              // Call the user-supplied out_function with this side
              // column's entry.  Note that column_index here is in
              // reference to data.rm, which is this side column's
              // data, not the overall column index; thus shift it by
              // the offset.
              out_function(mode, data.column_offset + column_index,
                           feature_index, value,
                           index_size, index_offset);
            };

            // Read this raw data into the user's out_function
            // (through sd_out_function_with_hook) and next_column.
            _read_raw_ml_data_column_block(
                data.rm, data.row_block_ptr, sd_out_function_with_hook, next_column);
          }
        }
      };
  ...
  // row_data_size, if not constant, does not include it's own index.

#ifndef NDEBUG
  if(rm.has_target)
    ++row_block_ptr;

  DASSERT_EQ(std::distance(row_block_start, row_block_ptr), row_data_size);
#endif
}
