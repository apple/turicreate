...
gl_sframe augment_data(const gl_sframe& data,
  ...
  if (verbose) {
    logprogress_stream << "Augmenting input images using " << backgrounds.size()
  ...
  for (const auto& row : data.range_iterator()) {
    input_row_index++;
    flex_image object =
        row[column_index_map[image_column_name]].to<flex_image>();
    std::string label =
        row[column_index_map[target_column_name]].to<flex_string>();
    ...
    for (const auto& background_ft : backgrounds.range_iterator()) {
      ...
      ParameterSampler parameter_sampler =
          ParameterSampler(background_width, background_height,
                           (background_width - object.m_width) / 2,
                           (background_height - object.m_height) / 2);

      flex_dict annotation =
          build_annotation(parameter_sampler, label, object.m_width,
                           object.m_height, seed + row_number);
      ...
      boost::gil::rgba8_image_t::view_t starter_image_view =
          create_starter_image_view(object);
      ...
      images.push_back(create_synthetic_image(
          starter_image_view, background_view, parameter_sampler, object));
      ...
      if (verbose) {
        ...
        // by 4.0
        int augmented_rows_completed =
            (input_row_index * backgrounds_size) + row_number;
  ...
  const std::map<std::string, std::vector<flexible_type> >& augmented_data = {
      {target_column_name, annotations}, {image_column_name, images}};
