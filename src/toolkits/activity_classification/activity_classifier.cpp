...
std::tuple<size_t, size_t> cumulative_chunk_accuracy(
  ...
  size_t cumulative_per_batch_chunk = 0;
  ...
  for (size_t i = 0; i < batch.batch_info.size(); ++i){
    ...
    cumulative_per_batch_chunk += num_correct_predictions;
    num_samples += info.num_samples;
  }

  return std::make_tuple(cumulative_per_batch_chunk, num_samples);
}
...
std::tuple<float, float> activity_classifier::compute_validation_metrics(
  ...
  size_t val_size = 0;
  size_t cumulative_val_chunk = 0;
  size_t num_val_samples = 0;
  size_t cumulative_val_chunks = 0;
  ...
  auto pop_until_size = [&](size_t remaining) {
    ...
    while (pending_batches.size() > remaining) {
      ...
      std::tie(cumulative_val_chunk, num_val_samples) =
          cumulative_chunk_accuracy(prediction_window, num_classes,
                                    batch.output_info, batch.data_info);
      cumulative_val_chunks += cumulative_val_chunk;
      total_val_samples += num_val_samples;
  ...
  float average_val_accuracy =
      static_cast<float>(cumulative_val_chunks) / total_val_samples;
...
void activity_classifier::perform_training_iteration() {
  ...
  size_t cumulative_train_chunk = 0;
  size_t cumulative_train_chunks = 0;
  ...
  auto pop_until_size = [&](size_t remaining) {
    ...
    while (pending_batches.size() > remaining) {
      ...
      std::tie(cumulative_train_chunk, num_train_samples) =
          cumulative_chunk_accuracy(prediction_window, num_classes,
                                    batch.output_info, batch.data_info);
      cumulative_train_chunks += cumulative_train_chunk;
  ...
  float average_batch_accuracy =
      static_cast<float>(cumulative_train_chunks) / total_train_samples;
  float average_val_accuracy;
