...
+ (JSValue *)wrap:(JSValue *)instance {
  return [TCVegaLogProxy wrap:instance withHandler:[[TCVegaLogProxyHandler alloc] init]];
}

+ (JSValue *)wrapObject:(NSObject *)object {
  // Assumes we have a current JSC context
  return [TCVegaLogProxy wrap:[JSValue valueWithObject:object inContext:[JSContext currentContext]]];
}
