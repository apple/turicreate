...
def make_clustering_data(n, d, seed=None):
    ...
    # dict column
    string_col = test_util.uniform_string_column(n, word_length=5,
                                                 alphabet_size=5,
                                                 missingness=0.)

    sf['dict0'] = tc.text_analytics.count_ngrams(string_col,
                                                 n=3, method='character',
                                                 to_lower=False)
...
class KmeansModelTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.model = tc.kmeans.create(self.sf, num_clusters=self.K,
    ...
    def test_get(self):
        ...
        self.assertItemsEqual(self.model.features,
    ...
    def test_predict_params(self):
        ...
        ans = self.model.predict(self.sf, output_type='cluster_id',
                                 verbose=False)
        self.assertIsInstance(ans, tc.SArray)
        self.assertTrue(ans.dtype == int)

        ans = self.model.predict(self.sf, output_type='distance',
                                 verbose=False)
        self.assertIsInstance(ans, tc.SArray)
...
class KmeansCreateTest(unittest.TestCase):
    ...
    def test_bad_data(self):
        ...
        with self.assertRaises(ValueError):
            m = tc.kmeans.create(dataset=tc.SFrame(), num_clusters=self.K,
                                 max_iterations=self.max_iter, verbose=False)

        ## Dataset as an SArray
        with self.assertRaises(TypeError):
            m = tc.kmeans.create(dataset=self.sf['int0'], num_clusters=self.K,
                                 max_iterations=self.max_iter, verbose=False)
    ...
    def test_bogus_parameters(self):
        ...
        with self.assertRaises(ValueError):
                m = tc.kmeans.create(dataset=self.sf, num_clusters=self.n + 1,
                                     verbose=False)

        ## Bad values for max_iterations
        for max_iter in [-1, 'fossa', 3.5]:
            with self.assertRaises(ToolkitError):
                m = tc.kmeans.create(dataset=self.sf, num_clusters=self.K,
    ...
    def test_features_param(self):
        ...
        self.assertItemsEqual(m.features,
            ['int0', 'int1', 'int2', 'float0', 'float1', 'float2', 'dict0'])

        ## The features parameter should be reflected correctly in the model.
        test_ftrs = ['int0', 'int1', 'int2']
        m = tc.kmeans.create(self.sf, num_clusters=self.K, features=test_ftrs,
                             verbose=False)
        self.assertItemsEqual(m.features, test_ftrs)

        ## Missing features are pruned
        m = tc.kmeans.create(self.sf, num_clusters=self.K,
                             features=['int0', 'int1', 'fossa'], verbose=False)
        ...
        m = tc.kmeans.create(sf, num_clusters=self.K, features=test_ftrs,
                             verbose=False)
        self.assertItemsEqual(m.features, ['int0', 'int1', 'int2'])
        ...
        m = tc.kmeans.create(sf, label='row_id', num_clusters=self.K,
                             features=test_ftrs, verbose=False)
        self.assertItemsEqual(m.features, ['int0', 'int1', 'int2'])

        ## Empty list of features raises an error.
        with self.assertRaises(ValueError):
            m = tc.kmeans.create(self.sf, features=[], num_clusters=self.K,
                                 verbose=False)

        ## All features pruned raises an error.
        test_ftrs = ['row_id', 'list0']
        with self.assertRaises(ToolkitError):
            m = tc.kmeans.create(sf, features=test_ftrs, label='row_id',
                                 num_clusters=self.K, verbose=False)
    ...
    def test_label_param(self):
        ...
        m = tc.kmeans.create(self.sf, num_clusters=self.K, verbose=False)
        self.assertItemsEqual(m.cluster_id.column_names(),
        ...
        m = tc.kmeans.create(sf, label=label_name, num_clusters=self.K,
                             verbose=False)
        self.assertItemsEqual(m.cluster_id.column_names(),
                              [label_name, 'cluster_id', 'distance'])
    ...
    def test_batch_size(self):
        ...
        ## Typical usage
        m = tc.kmeans.create(self.sf, num_clusters=self.K, batch_size=self.n / 5,
        ...
        m = tc.kmeans.create(self.sf, num_clusters=self.K, batch_size=2 * self.n,
                             max_iterations=10, verbose=False)
    ...
    def test_random_initial_centers(self):
        ...
        """
        ## Check the basics
        m = tc.kmeans.create(self.sf, num_clusters=self.K, max_iterations=0,
                             verbose=False)
...
class KmeansClusterTest(unittest.TestCase):
    ...
    def test_distance_accuracy(self):
        ...
        ## Create models.
        kmeans = tc.kmeans.create(self.sf, features=ftrs, num_clusters=3,
                                  verbose=False)
        knn = tc.nearest_neighbors.create(kmeans.cluster_info, features=ftrs,
                                          method='ball_tree',
                                          distance='euclidean', verbose=False)

        ## Transform int features into floats.
        coltype_map = {k: v for k, v in zip(self.sf.column_names(),
                                            self.sf.column_types())}
        sf_float = tc.SFrame()
        for ftr in ftrs:
        ...
        self.assertTrue((kmeans.cluster_id['cluster_id'] == \
                         knn_dists['reference_label']).all())
    ...
    def test_predictions(self):
        ...
        ystar = tc.SFrame({'cluster_id': ystar_labels,
        ...
        coltype_map = {k: v for k, v in zip(sf_predict.column_names(),
