...
def _annotate_image(pil_image, anns, confidence_threshold):
    ...
    for ann in reversed(anns):
        ...
        draw.rectangle(
            [
                (left - 1, label_top - height - 2 * BUF),
                (left + width + 2 * BUF, label_top),
            ],
            fill=color,
        )
