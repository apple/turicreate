...
class StandardDistancesTest(unittest.TestCase):
    ...
    def test_euclidean(self):
        self.assertAlmostEqual(euclidean(self.a, self.b),
                               tc.distances.euclidean(self.a, self.b))
        self.assertAlmostEqual((2*2 + 2*2 + 2*2)**.5,
                               tc.distances.euclidean(self.av, self.bv))

    def test_squared_euclidean(self):
        self.assertAlmostEqual(euclidean(self.a, self.b) ** 2,
                               tc.distances.squared_euclidean(self.a, self.b))

    def test_manhattan(self):
        self.assertAlmostEqual(manhattan(self.a, self.b),
                               tc.distances.manhattan(self.a, self.b))
    ...
    def test_jaccard(self):
        self.assertAlmostEqual(jaccard(self.a, self.b),
                               tc.distances.jaccard(self.a, self.b))
        self.assertAlmostEqual(jaccard(self.al, self.bl),
                               tc.distances.jaccard(self.al, self.bl))
    def test_weighted_jaccard(self):
        self.assertAlmostEqual(weighted_jaccard(self.a, self.b),
                               tc.distances.weighted_jaccard(self.a, self.b))
        self.assertAlmostEqual(weighted_jaccard(self.al, self.bl),
...
class DistanceUtilsTest(unittest.TestCase):
    ...
    def test_composite_feature_scrub(self):
        ...
        ans = tc.distances._util._scrub_composite_distance_features(dist,
                                                                   feature_blacklist)
        ...
        self.assertItemsEqual(tc.distances._util._get_composite_distance_features(ans),
                              ['a', 'b', 'c', 'd', 'e'])
    ...
    def test_composite_features_extract(self):
        ...
        self.assertItemsEqual(ans,
            tc.distances._util._get_composite_distance_features(dist))
...
class LocalDistancesTest(unittest.TestCase):
    ...
    def test_local_weighted_jaccard(self):
        ans = 1 - (0. + 0.7 + 0. + 0.) / (0.5 + 1.0 + 0.1 + 0.5)
...
def cosine(a, b):
    ...
    den = sum([v ** 2 for k, v in a.items()]) * \
          sum([v ** 2 for k, v in b.items()])
    den = den ** .5
    if den == 0:
        den = 0.0001
    return 1 - num/den
