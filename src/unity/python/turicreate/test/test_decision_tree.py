...
class DecisionTreeRegressionTest(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.model = tc.decision_tree_regression.create(self.dtrain,
                                             target=self.target,
                                             validation_set=self.dtest,
                                             **self.param)
    ...
    def test_create(self):
        model = tc.decision_tree_regression.create(self.dtrain, target='label',
                                        validation_set=self.dtest,
                                        **self.param)
        ...
        self.assertRaises(ToolkitError,
                          lambda:tc.decision_tree_regression.create(self.dtrain,
                        target='label_wrong', **self.param))
    ...
    def _test_regression_model(self, train, test, rmse_threshold, target='label'):
        # create
        model = tc.decision_tree_regression.create(train, target=target,
                                        validation_set=test,
                                        **self.param)
        # predict
    ...
    def test_predict_new_dictionary_key(self):
        ...
        new_data['dict_color_feature'] = \
                    new_data['cap-color'].apply(lambda x: {'cap-color': ord(x)})
        ...
        test['dict_color_feature'] = test['dict_color_feature'].apply(
            lambda x: dict(list(x.items()) + list({'cap-color2': x['cap-color']+1}.items())))

        model = tc.decision_tree_regression.create(train, target='label',
                                  **self.param)
        # Act.
...
def binary_classification_integer_target(cls):
    ...
    cls.model = tc.decision_tree_classifier.create(cls.dtrain,
                                         target=cls.target,
                                         validation_set = cls.dtest,
                                         **cls.param)
...
def multiclass_classification_integer_target(cls):
    ...
    num_examples_per_class = {c:(cls.dtrain[cls.target] == c).sum() for c in
            cls.model.classes}
    cls.get_ans['num_examples_per_class'] = lambda x: x == num_examples_per_class
...
class DecisionTreeClassifierTest(unittest.TestCase):
    ...
    def test_create(self):
        model = tc.decision_tree_classifier.create(self.dtrain, target='label',
                                        validation_set=self.dtest,
                                        **self.param)
        ...
        dtrain['label'] = 10
        self.assertRaises(ToolkitError,
                          lambda: tc.decision_tree_classifier.create(self.dtrain,
                            target='label_wrong', **self.param))
    ...
    def test_evaluate(self):
        ...
        ans_metrics = ["accuracy", "auc", "confusion_matrix", "f1_score",
    ...
    def _test_classifier_model(self, train, test, accuracy_threshold, target='label'):
        # create
        model = tc.decision_tree_classifier.create(train, target=target,
                                        validation_set=test,
                                        **self.param)
        # predict
    ...
    def test_predict_new_dictionary_key(self):
        ...
        model = tc.decision_tree_classifier.create(train, target='label',
                                  **self.param)
        # Act.
        y1 = model.predict(test)
        y2 = model.predict(list(test))
