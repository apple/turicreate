...
def make_classifier_data(n, d, seed=None):
    ...
    for i in range(d + 1):
        sf['str{}'.format(i)] = test_util.uniform_string_column(n, word_length=5,
                                                                alphabet_size=5,
                                                                missingness=0.)

    sf['dict0'] = tc.text_analytics.count_ngrams(sf['str{}'.format(d)],
                                                 n=3, method='character',
                                                 to_lower=False)
    sf.remove_column('str{}'.format(d), inplace=True)

    # target column (string)
    sf['class'] = test_util.uniform_string_column(n, word_length=1,
                                                  alphabet_size=3,
                                                  missingness=0.)
...
class KnnClassifierCreateTest(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.verbose = False
        self.distance = [[['int0', 'int1', 'float0', 'float1', 'array0'], 'euclidean', 1],
    ...
    def test_bad_data(self):
        ...
        with self.assertRaises(ToolkitError):
            m = tc.nearest_neighbor_classifier.create(dataset=tc.SFrame(),
                                                      target='class',
                                                      verbose=self.verbose)

        ## SArray for the features
        with self.assertRaises(ToolkitError):
            m = tc.nearest_neighbor_classifier.create(dataset=self.sf['int0'],
                                                      target='class',
                                                      verbose=self.verbose)

        ## SArray for the target
        with self.assertRaises(ToolkitError):
            m = tc.nearest_neighbor_classifier.create(dataset=self.sf,
                                                      target=self.sf['class'],
                                                      verbose=self.verbose)
    ...
    def test_features(self):
        ...
        m = tc.nearest_neighbor_classifier.create(self.sf[numeric_features + ['class']],
                                                  target='class',
                                                  distance='euclidean',
                                                  verbose=False)
...
class KnnClassifierModelTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.model = tc.nearest_neighbor_classifier.create(sf, target='class',
                                                           distance=self.distance,
                                                           verbose=False)
    ...
    def test_get(self):
        ...
        correct_fields = {'distance': self.distance,
...
class KnnClassifierPredictTest(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.model = tc.nearest_neighbor_classifier.create(self.sf,
                                                           target='class',
                                                           distance=self.distance,
                                                           verbose=False)
    ...
    def test_classify(self):
        ...
        ystar = self.model.classify(self.sf_test, max_neighbors=None, radius=15.,
                                    verbose=False)

        self.assertItemsEqual(ystar.column_types(), [str, float])
        self.assertTrue(ystar['class'].countna() > 0 and
                        ystar['class'].countna() < self.sf_test.num_rows())
        self.assertTrue(ystar['probability'].countna() > 0 and
                        ystar['probability'].countna() < self.sf_test.num_rows())
    ...
    def test_predict(self):
        ...
        ystar = self.model.predict(self.sf[:1], output_type='class',
                                   verbose=False)
        self.assertIs(ystar.dtype, str)

        ystar = self.model.predict(self.sf[:1], output_type='probability',
                                   verbose=False)
    ...
    def test_evaluate(self):
        ...
        ## Check that the results are correct if *some*, but not *all* of the
        #  test points have no qualified neighbors in the training set.
        evals = self.model.evaluate(self.sf_test, max_neighbors=None, radius=15.)
        acc = evals['accuracy']
        cf_mat = evals['confusion_matrix']
        ...
        self.assertTrue(cf_mat['predicted_label'].countna() > 0 and
                        cf_mat['predicted_label'].countna() < self.sf_test.num_rows())
