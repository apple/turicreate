...
class AdditionalDataTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        user_data['user_feature_value'] = [.5, .9]
        user_data['user_dict_value'] = [{1 : .5}, {4 : .9}]
        user_data['user_vect_value'] = [[0,1,2], [2,3,4]]
        user_data['user_str_dict_value'] = [{"tt" : .5}, {"ttt" : .9}]
        item_data = tc.SFrame()
        item_data['item_id'] = ['x', 'v', 'w', 'y']
        item_data['item_feature_value'] = [-.3, .7, .3, .05]
        item_data['item_dict_value'] = [{1 : .5}, {4 : .9}, {4 : .9}, {5 : 1, 6 : 2}]
        item_data['item_vect_value'] = [[0,1,2], [2,3,4], [2,3,4], [2,3,5] ]
        item_data['item_str_dict_value'] = [{"tt" : .5}, {"tt" : .9}, {"t" : .9}, {"ttt" : .9}]
        new_data = tc.SFrame()
        ...
        new_user_data['user_dict_value'] = [{1 : .5}, {4 : .9}]
        new_user_data['user_vect_value'] = [[0,1,2], [2,3,4]]
        new_user_data['user_str_dict_value'] = [{"tt" : .5}, {"ttt" : .9}]

        new_item_data = tc.SFrame()
        new_item_data['item_id'] = ['y', 'z']
        new_item_data['item_feature_value'] = [.5, .6]
        new_item_data['item_dict_value'] = [{1 : .5}, {4 : .9}]
        new_item_data['item_vect_value'] = [[0,1,2], [2,3,4]]
        new_item_data['item_str_dict_value'] = [{"tt" : .5}, {"ttt" : .9}]
    ...
    def _test_recommend(self, m):
        ...
        recs = m.recommend(users_all, top_k, exclude, items_all,
                           new_data, new_user_data, new_item_data,
                           exclude_known=True)
