...
class SVMClassifierTest(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.model = tc.svm_classifier.create(self.sf, target='target',
            features=None,
        ...
        classes = predictions
        margins = [np.concatenate(([1], x)).dot(np.array(self.coef)) \
                                                             for x in X_train]
        ...
        self.get_ans = {
         ...
         'convergence_threshold': \
                 lambda x: x == self.opts['convergence_threshold'],
         ...
         'num_examples_per_class': lambda x: {0: (tc.SArray(target) == 0).sum(),
    ...
    def test_evaluate(self):
        ...
        def check_cf_matrix(ans):
            ...
            sm = self.sf_margin['confusion_matrix']\
                     .sort(['target_label', 'predicted_label'])
            self.assertTrue(
                    np.allclose(cf['count'], sm['count']))
...
class SVMCreateTest(unittest.TestCase):
    ...
    def _test_create(self, sf, target, features, solver, kwargs):
        ...
        model = tc.svm_classifier.create(sf, target, features,
...
class ListCategoricalSVMTest(unittest.TestCase):
    ...
    def _test_create(self, sf, target, features, solver, kwargs):
        ...
        model = tc.svm_classifier.create(sf, target, features,
                              solver=solver, feature_rescaling = False,
                              **kwargs)
...
class CategoricalSVMTest(unittest.TestCase):
    ...
    def test_predict_new_categories(self):
      ...
      self.sf['species'] = self.sf['species'].apply(lambda x: 'rat' if x == 'foosa'
          else x)
      pred = model.evaluate(self.sf)
      self.sf['species'] = self.sf['species'].apply(lambda x: 'foosa' if x == 'rat'
          else x)
...
class VectorSVMTest(unittest.TestCase):
  ...
  def _test_create(self, sf, target, features, solver,
    ...
    model = tc.svm_classifier.create(sf, target, features, solver = solver,
                          feature_rescaling = False, **kwargs)
...
class DictSVMTest(unittest.TestCase):
  ...
  def _test_create(self, sf, target, features, solver,
    ...
    model = tc.svm_classifier.create(sf, target, features, solver
        = solver, feature_rescaling = False, **kwargs)
  ...
  def test_predict_extra_cols(self):
    ...
    sf = self.sf[:]
    model = tc.svm_classifier.create(sf, self.target, self.features,
        feature_rescaling = False)
    pred = model.predict(sf)
    sf['dict'] = sf['dict'].apply(lambda x: dict(list(x.items())
                             + list({'extra_col': 0}.items())))
