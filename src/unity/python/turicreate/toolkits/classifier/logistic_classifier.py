...
class LogisticClassifier(_Classifier):
    ...
    def evaluate_with_predictions(self, dataset, metric='auto', missing_value_action='auto'):
        """
        Evaluate the model by making predictions of target values and comparing
        ...
        .. sourcecode:: python
          ...
          ...                             target='is_expensive',
          ...                             features=['bath', 'bedroom', 'size'])
          >>> results = model.evaluate_with_predictions(data)
          >>> print results['accuracy']
        """
        ...
        _raise_error_evaluation_metric_is_valid(metric,
                ['auto', 'accuracy', 'confusion_matrix', 'roc_curve', 'auc',
                 'log_loss', 'precision', 'recall', 'f1_score'])
        return super(_Classifier, self).evaluate_with_predictions(dataset,
                                 missing_value_action=missing_value_action,
                                 metric=metric)
