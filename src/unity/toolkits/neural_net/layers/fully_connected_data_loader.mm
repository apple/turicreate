...
- (id _Nonnull)initWithParams:(NSString *_Nonnull)name
  ...
  if (weights != NULL) {
    ...
    mWeights = [dev newBufferWithBytes:weights
                                length:sizeof(float) * mInputFeatureChannels *
                                       mKernelHeight * mKernelHeight *
                                       mOutputFeatureChannels
                               options:MTLResourceStorageModeManaged];
  } else {
    mWeights = [dev newBufferWithLength:sizeof(float) * mInputFeatureChannels *
  ...
  mWeightMomentumBuffer = [dev
      newBufferWithLength:sizeof(float) * mInputFeatureChannels *
                          mKernelHeight * mKernelWidth * mOutputFeatureChannels
                  options:storageMode];

  mWeightVelocityBuffer = [dev
      newBufferWithLength:sizeof(float) * mInputFeatureChannels *
                          mKernelHeight * mKernelWidth * mOutputFeatureChannels
                  options:storageMode];

  MPSVectorDescriptor *vDescWeights = [MPSVectorDescriptor
      vectorDescriptorWithLength:mInputFeatureChannels * mKernelHeight *
                                 mKernelWidth * mOutputFeatureChannels
                        dataType:(MPSDataTypeFloat32)];

  mWeightMomentumVector =
      [[MPSVector alloc] initWithBuffer:mWeightMomentumBuffer
                             descriptor:vDescWeights];

  mWeightVelocityVector =
      [[MPSVector alloc] initWithBuffer:mWeightVelocityBuffer
                             descriptor:vDescWeights];
  ...
  mState =
      [[MPSCNNConvolutionWeightsAndBiasesState alloc] initWithWeights:mWeights
                                                               biases:mBiases];
...
- (MPSCNNConvolutionWeightsAndBiasesState *_Nullable)
    updateWithCommandBuffer:(id<MTLCommandBuffer> _Nonnull)commandBuffer
              gradientState:
                  (MPSCNNConvolutionGradientState *_Nullable)gradientState
                sourceState:(MPSCNNConvolutionWeightsAndBiasesState *_Nullable)
                                sourceState {
  [mOptimizer
...
- (size_t)weight_size {
  return mKernelWidth * mKernelHeight * mInputFeatureChannels *
         mOutputFeatureChannels * sizeof(float);
}
