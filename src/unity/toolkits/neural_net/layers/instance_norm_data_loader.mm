...
- (id)initWithParams:(NSString *)name
  ...
  @autoreleasepool {
    ...
    std::vector<float> zeros(mNumberOfFeatureChannels, 0);
    ...
    vDesc = [MPSVectorDescriptor
        vectorDescriptorWithLength:mNumberOfFeatureChannels
                          dataType:(MPSDataTypeFloat32)]; //

    mGammaBuffer =
        [dev newBufferWithBytes:mGamma[0]
                         length:sizeof(float) * mNumberOfFeatureChannels
                        options:MTLResourceStorageModeManaged];

    mBetaBuffer =
        [dev newBufferWithBytes:mBeta[0]
                         length:sizeof(float) * mNumberOfFeatureChannels
                        options:MTLResourceStorageModeManaged];

    mGammaMomentumBuffer =
        [dev newBufferWithBytes:zeros_ptr
                         length:sizeof(float) * mNumberOfFeatureChannels
                        options:MTLResourceStorageModeManaged];

    mGammaVelocityBuffer =
        [dev newBufferWithBytes:zeros_ptr
                         length:sizeof(float) * mNumberOfFeatureChannels
                        options:MTLResourceStorageModeManaged];
    ...
    mMovingVarianceBuffer =
        [dev newBufferWithBytes:ones_ptr
                         length:sizeof(float) * mNumberOfFeatureChannels
                        options:MTLResourceStorageModeManaged];
    ...
    mGammaMomentumVector =
        [[MPSVector alloc] initWithBuffer:mGammaMomentumBuffer
                               descriptor:vDesc];

    mGammaVelocityVector =
        [[MPSVector alloc] initWithBuffer:mGammaVelocityBuffer
                               descriptor:vDesc];
    ...
    mState = [[MPSCNNNormalizationGammaAndBetaState alloc]
        initWithGamma:mGammaBuffer
                 beta:mBetaBuffer];
...
- (MPSCNNNormalizationGammaAndBetaState *)
  ...
  for (MPSCNNInstanceNormalizationGradientState
           *instanceNormalizationState in instanceNormalizationStateBatch) {
    MPSVector *gradientWeightsVector = [[MPSVector alloc]
        initWithBuffer:nonnull_cast_a(
                           instanceNormalizationState.gradientForGamma)
            descriptor:vDesc];

    MPSVector *inputWeightsVector = [[MPSVector alloc]
        initWithBuffer:nonnull_cast_a(instanceNormalizationState.gamma)
            descriptor:vDesc];
    ...
    MPSVector *gradientBiasesVector = [[MPSVector alloc]
        initWithBuffer:nonnull_cast_a(
                           instanceNormalizationState.gradientForBeta)
            descriptor:vDesc];

    MPSVector *inputBiasesVector = [[MPSVector alloc]
        initWithBuffer:nonnull_cast_a(instanceNormalizationState.beta)
            descriptor:vDesc];
