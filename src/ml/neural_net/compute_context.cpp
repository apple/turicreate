...
compute_context::registration::registration(int priority, factory factory_fn, factory tf_factory_fn,
                                            factory mlc_factory_fn)
  : priority_(priority)
  , factory_fn_(std::move(factory_fn))
  , tf_factory_fn_(std::move(tf_factory_fn))
  , mlc_factory_fn_(std::move(mlc_factory_fn))
{
...
// static
std::unique_ptr<compute_context> compute_context::create_mlc()
{
  // Return the tensorflow compute context only.
  std::unique_ptr<compute_context> result;
  auto it = get_registry().begin();
  while (result == nullptr && it != get_registry().end()) {
    result = it->second->create_mlc_context();
    ++it;
  }
  return result;
}
