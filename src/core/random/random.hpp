...
namespace turi {
  ...
  namespace random {
    ...
    namespace distributions {
      ...
      struct uniform {
        typedef std::uniform_int_distribution<IntType> distribution_type;
    ...
    public:
      ...
      void seed(generator& other){
        uint32_t _seed = other.uniform<uint32_t>(0, std::numeric_limits<uint32_t>::max());
        seed(_seed);
      }
      ...
      inline NumType uniform(const NumType min, const NumType max,
        ...
        std::uniform_int_distribution<NumType> d(min, max);
      ...
      inline NumType uniform(const NumType min, const NumType max, 
        ...
        std::uniform_real_distribution<NumType> d(min, max);
        std::lock_guard<std::mutex> lg(mut);
        return d(m_rng);
      } // end of uniform 
      ...
      inline double gamma(const double alpha = double(1)) {
        std::gamma_distribution<double> gamma_dist(alpha);
        std::lock_guard<std::mutex> lg(mut);
        const double result = gamma_dist(m_rng);
        return result;
      } // end of gamma
      ...
      inline double gaussian(const double mean = double(0),
        ...
        mut.lock();
        const double result = normal_dist(m_rng);
        mut.unlock();
        return result;
      } // end of gaussian
      ...
      inline double cauchy(const double location = double(0),
        ...
        std::cauchy_distribution<double> cauchy_dist(location, scale);
        mut.lock();
        const double result = cauchy_dist(m_rng);
        mut.unlock();
        return result;
      } // end of cauchy
      ...
      inline bool bernoulli(const double p = double(0.5)) {
        ...
        mut.lock();
        const double result(dist(m_rng));
        mut.unlock();
        return result;
      } // end of bernoulli
      ...
      size_t multinomial(const VecType& prb, VType norm) {
        ...
        if(norm < 1e-20) {
          return uniform<VType>(0, prb.size() - 1);
        }
        ...
        VType rnd = uniform<VType>(0,norm - (std::is_integral<VType>::value ? 1 : 0));
